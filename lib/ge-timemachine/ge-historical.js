...
message DbRootProto { optional DatabaseVersionProto database_version = 13; }
...
function parseRootQuadtreeVersion(u8) {
  const m = T_DbRoot.decode(u8);
  return m.databaseVersion?.quadtreeVersion ?? 1;
}
...
export class GEHistoricalSource {
  ...
  async _loadDbRoot() {
    ...
    this.rootEpoch = parseRootQuadtreeVersion(uncompress(body)) || 1;
    this._log("dbRoot ok — keyLen", this.key.byteLength, "rootEpoch", this.rootEpoch);
    return this;
  }
    ...
    if (node) {
      ...
      if (histTiles && histTiles.length) {
        for (const dt of histTiles) {
          ...
          else if (imagery) dated.push({ ...d, epoch: imagery.layerEpoch ?? 0, provider: 0, kind: "default" });
        }
      } else if (imagery) {
        dated.push({ year: 0, month: 0, day: 0, packed: 0, epoch: imagery.layerEpoch ?? 0, provider: 0, kind: "default" });
      }
    }
