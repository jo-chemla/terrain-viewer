...
**UI polish:** the historical timeline's zoom-out limit is now padded 10% beyond the true oldest/newest tick on each side (20% total) so those marks don't sit pinned exactly at the track's 0%/100% edge when fully zoomed out. The Maxar VHR/Medium-resolution toggle was removed from the Basemap section entirely (per Jonathan's call to simplify) — `lib/maxar.ts`'s historical branch now always requests the `"vhr"` tier; the underlying function still accepts a tier param for whenever this needs reintroducing, but nothing in the UI/state layer surfaces it anymore (`maxarResolutionTierAtom` deleted).
