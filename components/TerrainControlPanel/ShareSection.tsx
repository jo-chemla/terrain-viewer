...
import { Share2, Check, ImageIcon, Twitter, Linkedin, Instagram } from "lucide-react"
...
const PLATFORMS: SharePlatform[] = [
    ...
    },
    {
        ...
        icon: SiLinkedin,
        color: "#0a66c2",
        // LEGACY shareArticle supports url + title + summary + source
        // buildUrl: (text, pageUrl, title) =>
        //     `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(pageUrl)}&title=${encodeURIComponent(title)}&summary=${encodeURIComponent(text)}&source=terrain-viewer.iconem.com`,
...
const ShareModal: React.FC<{
...
}> = ({ open, onOpenChange, getScreenshotBase64 }) => {
    ...
    const getOrCreateBlob = useCallback(async (): Promise<Blob | null> => {
        ...
        if (!getScreenshotBase64) return null
            ...
            <DialogContent className="sm:max-w-sm bg-background border border-border [&>button]:cursor-pointer">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-xs text-muted-foreground">
                        Click a platform to open its composer. Your screenshot is copied to clipboard
                        automatically — press{" "}
          ...
          "
                ...
                "
                            >
                                ...
                                // style={{ backgroundColor: `${platform.color}2`, color: platform.color }}
                                >
                ...
                <p className="text-[10px] text-muted-foreground/40 text-center">
                    Shares the current URL with all your map settings
                </p>
            </DialogContent>
        </Dialog>
    )
...
export const ShareButton: React.FC<{
    ...
    const getScreenshotBase64 = useCallback(async (): Promise<string | null> => {
        if (!mapRef?.current) return null
        try {
            const { domToPng } = await import("modern-screenshot")
            const canvas = mapRef.current.getMap().getCanvas()
            const dataUrl: string = await domToPng(canvas, {
    ...
    return (
        <>
            ...
            <ShareModal open={open} onOpenChange={setOpen} getScreenshotBase64={getScreenshotBase64} />
        </>
    )
}