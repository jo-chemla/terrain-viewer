...
#### TL;DR
- **Sidebar/timeline avoidance padding now applies to every pane it should** — a single-row grid, a single-column grid, the lone non-split view, and both Overlay panes all sit under the sidebar and/or the timeline panel, and all now get the matching camera-padding correction so the focus point stays centered in the *visible* part of the pane. Previously these cases were skipped, so the camera jumped sideways when toggling the sidebar or switching to Overlay.
- **Fixed split-view pan/drag desync** — a sync-guard timeout was silently dropping most real drag frames, letting the non-dragged view lag a few frames behind (visibly, on 3D terrain) until an extra nudge resynced it.
- **Fixed split views drifting vertically apart *during* a drag** (snapping back into alignment the moment it ended) — a terrain camera has a sixth parameter beyond center/zoom/bearing/pitch/roll, the elevation of the point it looks at, and it wasn't being synced. MapLibre updates it by two different rules mid-drag: the dragged view is elevation-frozen for the whole gesture, while every synced view's `jumpTo` re-resolves it against live ground. The moved view's elevation is now forwarded explicitly to the others.
- **Fixed terrain camera mis-centering** (e.g. the Matterhorn summit sitting visibly low/off-center) on load *and* on every Off/Overlay/Side split-mode switch — DEM tiles arriving after terrain is switched on, pane resizes, and the sidebar-avoidance padding transition each leave the camera's elevation reference stale. Now re-resolved on `idle` and immediately after a split-mode switch.
- **Fixed terrain visibly blinking through several states as it loads** — the elevation fix above used to be a full `setTerrain()` re-application on every idle, which rebuilds MapLibre's Terrain and RenderToTexture objects and throws away the render-to-texture tile cache each time; stacked `sourcedata` listeners piled up more of the same. It's now a single DEM lookup that MapLibre no-ops when nothing changed, and terrain is only re-applied when the bound source or exaggeration actually differs.
- **Fixed "weird follow-terrain" while dragging** — MapLibre's own live ground-elevation reclamping (on by default) was re-resolving the camera's height every rendered frame, causing a bob/climb effect. Now disabled once each view's terrain has settled.
- **Fixed the camera lurching a moment after a pan settles** (single view included) — the terrain re-anchor now uses MapLibre's own `recalculateZoomAndCenter`, which holds the camera position fixed and solves for the center/zoom that put the target back on the ground, so nothing visibly moves. It only runs when the height is genuinely stale (i.e. after a pan across terrain), so padding changes, resizes and split-mode switches no longer nudge the focus point off its coordinates. In split/overlay the view you last touched is the one re-anchored, and every other view is copied wholesale from it — so the panes can never drift apart from whichever one you're driving.
- **Fixed one pane following the terrain while you panned the *other* one** — the "clamping is off for this view" bookkeeping was keyed by view id, but a view id outlives the map behind it (toggling split mode off and on builds a brand new map for view B), so a fresh pane could be left permanently ground-clamped while its flag claimed otherwise. Compounding it, MapLibre's `easeTo` leaves its internal elevation freeze stuck on unless `freezeElevation` is passed, and only a completed drag *on that map* ever clears it — so the two panes ended up in different internal states depending on which one you'd last dragged. Both are now handled from the map's own live state rather than remembered per view id.
- **Fixed a map pane randomly refusing to drag** — the cursor was right, but the press did nothing until you released and tried again. MapLibre's `jumpTo`/`easeTo` begin with `Map#stop()`, which resets every input handler on that map; issuing one while a pointer was already down (but hadn't moved yet, so nothing looked "in progress") silently ate the gesture. It could latch, too, since the killed pane never emits a move and so never becomes the one driving the sync. Programmatic camera commands now stand down while a pointer is held, and the terrain settle skips propagating entirely when there was nothing to correct — which is almost always.
- **Panel open/close is now animated end to end** — the map's vanishing-point padding, the pane rects, the grid gutter, the borders and the seams all ease over the same 200ms instead of the camera snapping while the panel slides. Dragging the split divider and resizing the window stay instantaneous.
- Split mode's Map B now defaults to the **AWS terrain source** (no API key required) instead of MapTiler, so a fresh visitor's split view never renders a blank pane.
- New docs page: **Terrain Sources**, covering all 4 built-in DEM sources (Mapterhorn, Mapbox, MapTiler, AWS), linked from each source's info dialog.
- Visiting `beta.historical-satellite.iconem.com` now reliably defaults to **Historical Imagery mode** (previous fix had an effect-ordering race), and app/docs favicons now correctly default blue in prod / purple in dev, with the app's icon *shape* reactive to the live app mode rather than just the hostname.
...
### Bug Fixes
- **Camera/focus-point shift on init in split/grid mode** — right (sidebar-avoidance) and bottom (timeline-avoidance) padding were computed from `isSplit` alone, so a single-row or single-column grid still got padding meant for a genuinely multi-row/column layout, nudging every view's center away from the intended focus point. Now only applied when the grid actually has more than one row (bottom) or more than one column (right).
