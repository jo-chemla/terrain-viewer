...
#### TL;DR
- **Fixed the on-load camera/focus-point shift** in split/grid mode — right/bottom sidebar-and-timeline avoidance padding was still being applied for a single-row or single-column grid, nudging the view off-center even when there was nothing to avoid.
...
- **Fixed terrain camera mis-centering** (e.g. the Matterhorn summit sitting visibly low/off-center) on load *and* on every Off/Overlay/Side split-mode switch — several distinct MapLibre operations (`setTerrain`, `easeTo`/`jumpTo`, `resize()`) each independently reset the camera's terrain-elevation reference, with nothing reliably re-resolving it afterward. Now corrected uniformly via a persistent per-view `idle` listener, verified live across a full Off → Overlay → Side → Off cycle.
- **Fixed "weird follow-terrain" while dragging** — MapLibre's own live ground-elevation reclamping (on by default) was re-resolving the camera's height every drag frame, causing a bob/climb effect that wasn't replicated identically between split views using different DEM sources. Now disabled after each view's own one-time correct settle.
